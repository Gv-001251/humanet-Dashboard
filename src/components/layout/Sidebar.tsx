        ...
        <div className="border-t border-neutral-border p-4">
          <div className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-primary/10 text-sm font-semibold text-brand-primary">
              {DEFAULT_USER.name.split(' ').map((n) => n[0]).join('').slice(0, 2).toUpperCase()}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-neutral-text">{DEFAULT_USER.name}</p>
              <p className="text-xs text-neutral-muted capitalize">{DEFAULT_USER.role}</p>
            </div>
          </div>
        </div>
      </div>
